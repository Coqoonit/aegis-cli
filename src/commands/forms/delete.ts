...
export default defineCommand({
  meta: {
    name: "delete",
    description: "Delete a form. Requires --yes or TTY confirmation.",
  },
  args: {
    "form-id": {
      type: "positional",
    ...
    try {
      ...
      if (!args["dry-run"]) {
        await confirmDestructive({
          message: `Delete form ${formId} from case ${caseId}?`,
          yes: args.yes,
        });
      }

      await apiFetch(
        `/aml-cases/${encodeURIComponent(caseId)}/forms/${encodeURIComponent(formId)}`,
        {
          method: "DELETE",
          dryRun: args["dry-run"],
          pretty: args.pretty,
