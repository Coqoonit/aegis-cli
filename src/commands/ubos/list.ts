...
import { ListUBOsQuerySchema } from "../../schemas/ubo.js";

export default defineCommand({
  meta: {
    name: "list",
    description: "List Ultimate Beneficial Owners (UBOs) of a case.",
  },
  args: {
  ...
  async run({ args }) {
    try {
      ...
      const query = ListUBOsQuerySchema.parse({
        cursor: args.cursor,
        limit: args.limit ?? "100",
      });
      const qs = buildQueryString({ ...query });
      const res = await apiFetch<unknown>(
        `/aml-cases/${encodeURIComponent(caseId)}/ubos${qs}`,
      );
