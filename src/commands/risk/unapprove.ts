...
export default defineCommand({
  meta: {
    name: "unapprove",
    description:
      "Revoke approval of a risk assessment (returns to COMPLETED status).",
  },
  args: {
    "ra-id": {
      type: "positional",
  ...
  async run({ args }) {
    try {
      ...
      const res = await apiFetch<unknown>(
        `/aml-cases/${encodeURIComponent(caseId)}/risk-assessments/${encodeURIComponent(raId)}/unapprove`,
        {
          method: "POST",
          dryRun: args["dry-run"],
          pretty: args.pretty,
